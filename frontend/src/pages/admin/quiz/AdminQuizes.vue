...
async function initialFetch(){
    ...
    let res = await api.get(`/admin/subjects/${props.sid}/chapters/${props.cid}/quizes/?filter=${filter}&page=${page}&per_page=${per_page}&q=${query_str.value}`);
...
<template>
    <div v-if="quizes" class="d-flex flex-column flex-grow-1 mt-2 align-items-center">
        <form @submit.prevent="" class="d-flex flex-column w-100 align-items-center mt-1">
            <RouterLink :to="`/admin/subjects/${props.sid}/chapters/${props.cid}`">
                <h2>Back to chapter details</h2>
            </RouterLink>
            <h1 class="mb-2">
                Searching for quizes!
            </h1>
        ...
        <div class="d-flex flex-column align-items-center w-100">
            <div class="d-flex w-75">
                <input class="d-flex flex-grow-1" type="text" v-model="query_str" placeholder="Description">
